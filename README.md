...
Windows 알림 영역(시스템 트레이)에 상주하는 앱을 실행할 수 있다. 트레이 아이콘을 클릭하면 React 화면이 작은 앱 창처럼 열리고, 작업창에서 빠른 2A/2B MVP 기능을 바로 사용할 수 있다. 일반 앱 실행 모드는 긴글 첨삭과 리포트 화면만 띄운다.

- 추천받기: 무슨 말을 해야 할지 모를 때 상황/상대/목적/말투를 입력하면 후보 문장 3개를 생성
- 말투 점검: 이미 쓴 문장을 상황에 맞게 Mirror 분석
- 긴글 첨삭: 일반 앱 모드에서 논문/메일/편지/블로그/기타 유형에 맞춰 긴 글을 첨삭하고 자동 요약
...
긴글 첨삭/리포트용 일반 앱 창은 아래처럼 실행한다.

```powershell
.\.venv\Scripts\refiner-document.exe
```
...
.\.venv\Scripts\pythonw.exe -m refiner.windows_tray --document
```

트레이 앱 실행 후에는 작업 표시줄 오른쪽의 숨겨진 아이콘 영역에서 Magic Note 아이콘을 찾고, 아이콘을 클릭해 작은 작업창을 연다.
