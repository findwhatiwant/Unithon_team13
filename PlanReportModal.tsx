...
const CORRECTIONS = [
  {
    ...
    after: "확인 부탁드립니다",
    description: "배려를 표현하려는 좋은 의도이지만, 쿠션어가 겹치면 요청의 핵심이 흐려지고 자신 없어 보일 수 있어요. 먼저 필요한 일을 분명하게 말하고, 꼭 필요한 경우에만 부드러운 표현을 한 번 덧붙여 보세요.",
  },
  {
    habit: "2. 확신을 낮추는 문장 끝맺음",
    before: "제 생각에는 아마 이게 맞을 것 같아요",
    after: "이 방향이 맞다고 판단됩니다",
    description: "'아마', '것 같아요'가 반복되면 충분히 검토한 내용도 조심스럽거나 책임을 피하는 인상으로 들릴 수 있어요. 근거가 있을 때는 판단을 또렷하게 말하고, 불확실한 부분만 따로 밝혀 주세요.",
  },
  {
    habit: "3. 비유와 감정을 앞세우는 강조",
    before: "일정이 눈 깜짝할 사이에 폭탄처럼 다가올 것 같아요",
    after: "일정이 임박해 우선순위 조정이 필요합니다",
    description: "생생한 비유는 기억에 남지만, 업무나 요청 상황에서는 실제 문제와 필요한 행동을 가릴 수 있어요. 비유 대신 현재 상황, 영향, 다음 행동을 차례로 적으면 더 차분하고 설득력 있게 전달됩니다.",
  },
];

/* 외부 차트 라이브러리 없이 순수 SVG로 그리는 막대그래프 */
...
function PlanReportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // 모달을 다시 열 때마다 State 1(티저)부터 보여주기 위한 리셋
  useEffect(() => {
    if (open) setState(1);
  }, [open]);
        ...
        {state === 1 && (
          <div className="prm-state prm-state-1">
            <p className="prm-lead">
              지금까지 쓰신 <strong>128개</strong>의 문장을 분석해서 <br />무의식적으로 사용하는 습관들을 발견했어요.
            </p>
        ...
        {state === 2 && (
          <div className="prm-state prm-state-2">
            <h2 className="prm-title">
              최근 작성한 문장들을 분석해<br />반복되는 핵심 습관을 발견했어요.
            </h2>
        ...
        {state === 3 && (
          <div className="prm-state prm-state-3">
            ...
            <div className="prm-correction-list">
              {CORRECTIONS.map((c, i) => (
                <div className="prm-correction-row" key={i}>
                  <p className="prm-correction-habit">{c.habit}</p>
                  <p className="prm-correction-description">{c.description}</p>
                  <div className="prm-correction-example">
                    <span className="prm-before">{c.before}</span>
                    <span className="prm-arrow">➔</span>
                    <span className="prm-after">{c.after}</span>
                  </div>
                </div>
              ))}
            </div>
